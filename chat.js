...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const prompt = `You are a Catholic liturgical expert. Return the complete Roman Catholic Mass readings for ${date} as a JSON array. Return ONLY valid JSON, no markdown, no backticks, no explanation.
...
Rules: Include Second Reading only on Sundays and solemnities. Always include full text. Use NABRE-style English.`;
    ...
    return res.status(500).json({ error: "Failed to load readings. Please retry." });
  }
};
